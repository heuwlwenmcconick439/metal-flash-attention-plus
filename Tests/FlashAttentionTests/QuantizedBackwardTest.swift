...
final class QuantizedBackwardTest: XCTestCase {
    ...
    func testBackwardQueryStrategyEncoding() throws {
        ...
        let gradOutputBuffer = device.makeBuffer(length: tensorSize * 4, options: .storageModeShared)!
        let logsumexpBuffer = device.makeBuffer(length: Int(seqLength) * 4, options: .storageModeShared)!
        let gradQueryBuffer = device.makeBuffer(length: tensorSize * 4, options: .storageModeShared)!
        let dValuesBuffer = device.makeBuffer(length: Int(seqLength) * 4, options: .storageModeShared)!
    ...
    func testBackwardKeyValueStrategyEncoding() throws {
        ...
        let gradValueBuffer = device.makeBuffer(length: tensorSize * 4, options: .storageModeShared)!
