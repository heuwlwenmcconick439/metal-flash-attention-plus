  ...
  func testMultiHeadQuantizationStrategyEncoding() throws {
    ...
    for (testName, quantParams) in testConfigs {
      ...
      for binding in bindings {
        ...
        XCTAssertEqual(
          binding.parameters.scale,
          expectedParams.scale,
          "\(testName): Scale mismatch for \(binding.operand)"
        )
        XCTAssertEqual(
          binding.parameters.zeroPoint,
          expectedParams.zeroPoint,
          "\(testName): Zero point mismatch for \(binding.operand)"
        )
        XCTAssertEqual(
          binding.parameters.strategy,
          expectedParams.strategy,
          "\(testName): Strategy mismatch for \(binding.operand)"
        )
        XCTAssertEqual(
          binding.parameters.strategyVersion,
          expectedParams.strategyVersion,
