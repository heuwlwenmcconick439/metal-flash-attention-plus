...
public class GEMMRuntimeQuantization {
    ...
    public func quantizeBlockwiseCentered(
        ...
        guard blockSizeK > 0 && blockSizeK % 8 == 0 else {
            throw QuantizationError.invalidParameters("blockSizeK must be positive and multiple of 8")
        }
        ...
        default:
            throw QuantizationError.invalidParameters("Unsupported input precision: \(inputPrecision)")
        }
    ...
    public func createBlockwiseBuffers(
        ...
        guard let zeroPointsBuffer = device.makeBuffer(
    ...
    public func extractBlockwiseParameters(
        ...
        let zeroPoints = Array(UnsafeBufferPointer(start: zeroPointsPtr, count: numBlocks)).map { Int32($0) }
