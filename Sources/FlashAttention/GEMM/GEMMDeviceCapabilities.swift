...
public extension MTLDevice {
    ...
    func quantizationStrategy(requested: QuantizationMode) -> QuantizationStrategyResult {
        ...
        case .blockwise(let blockSizeK, let bothOperands):
            ...
            if !supportsBlockwiseQuantization {
                warnings.append("Device does not support efficient blockwise quantization (requires Apple7+ with simdgroup matrix operations)")

                if supportsBFloat16 {
                    warnings.append("Falling back to BFloat16 GEMM for better performance than INT8 without hardware acceleration")
            ...
            if blockSizeK % 8 != 0 {
                warnings.append("Block size \(blockSizeK) is not a multiple of 8; rounding to \(((blockSizeK + 7) / 8) * 8)")
            ...
            if blockSizeK > simdgroupSize * 4 {
                warnings.append("Block size \(blockSizeK) may be too large for optimal performance (simdgroup size: \(simdgroupSize))")
                warnings.append("Consider using block size ≤ \(simdgroupSize * 4) for better cache utilization")
            }
        ...
        case .rowWise:
            ...
            if !supportsSimdgroupMatrixOps {
                warnings.append("Device lacks simdgroup matrix operations; row-wise quantization may have reduced performance")
        ...
        case .tensorWise:
            ...
            if !supportsSimdgroupMatrixOps {
                warnings.append("Device lacks advanced matrix operations; performance may be limited")
            }
    ...
    func getQuantizationFallbackStrategy(for originalMode: QuantizationMode) -> QuantizationFallbackStrategy {
        switch originalMode {
        case .blockwise(_, let bothOperands):
            if supportsSimdgroupMatrixOps {
                // Can still do some blockwise operations, just not fully optimized
                return .symmetricBlockwise(weights: true, activations: bothOperands)
...
public extension QuantizationMode {
    ...
    func optimizedForDevice(_ device: MTLDevice) -> QuantizationStrategyResult {
        return device.quantizationStrategy(requested: self)
    }
    ...
    private func isEqual(to other: QuantizationMode) -> Bool {
        ...
        case (.rowWise, .rowWise):
            return true
        case (.blockwise(let size1, let both1), .blockwise(let size2, let both2)):
            return size1 == size2 && both1 == both2
        default:
            return false
        }
    }
