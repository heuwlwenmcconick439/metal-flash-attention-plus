...
struct BlockwiseQuantizedOperand: KBlockAligned {
  ...
  func validateAlignment(with other: KBlockAligned) throws {
    guard blockSizeK == other.blockSizeK else {
...
enum BlockwiseGEMMValidator {
  ...
  static func validateOperands(
    _ a: BlockwiseQuantizedOperand,
    _ b: BlockwiseQuantizedOperand
  ) throws {
...
enum BlockwiseQuantizationError: Error, LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case let .mismatchedBlockSizes(a, b):
      "Block sizes don't match: operand A has block size \(a), operand B has block size \(b)"
    case let .invalidBlockSize(size):
      "Invalid block size: \(size). Block size must be positive and typically a power of 2"
    case let .misalignedKDimension(k, blockSize):
      "K dimension \(k) is not properly aligned with block size \(blockSize)"
    case let .mismatchedKDimensions(aK, bK):
      "K dimensions don't match: operand A has K=\(aK), operand B has K=\(bK)"
    case let .mismatchedBlockCounts(aBlocks, bBlocks):
      "Block counts don't match: operand A has \(aBlocks) blocks, operand B has \(bBlocks) blocks"
    case let .insufficientBufferSize(buffer, expected, actual):
      "Buffer '\(buffer)' is too small: expected \(expected) bytes, got \(actual) bytes"
    }
  }
}

/// Configuration for blockwise quantization parameters
enum BlockwiseQuantizationConfig {
  ...
  static func isValidBlockSize(_ blockSize: Int) -> Bool {
    blockSize > 0 && blockSize.isPowerOfTwo
  }
