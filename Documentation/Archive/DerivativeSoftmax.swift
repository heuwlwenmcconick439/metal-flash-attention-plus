...
struct DerivativeSoftmaxKernel {
  ...
  init(
    ...
    threadgroupSize = 32 * splits.M * splits.N
    ...
    do {
      var asyncIterationsStart = if descriptor.preferAsyncLoad {
        "0"
      } else {
        "(K - (K % K_group))"
      }
    ...
    do {
      var memoryNameP = if memoryPrecisions.C == .BF16 {
        "half"
      } else {
        "float"
      }
    ...
    do {
      var storeFunctionC = if memoryPrecisions.C == .BF16 {
        "store_bfloat"
      } else {
        "store"
      }
